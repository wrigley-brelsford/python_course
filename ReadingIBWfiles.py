...
Output=TranslateObj.translate(file_path=r'C:\Users\ppxjbw\Documents\AFM\November\05.11.2019_JW\BN230_Regmac1_0000.ibw', verbose=False)
...
phase_Trace=h5_File['Measurement_000/Channel_003/Raw_Data']
...
ver_grad_array = np.linspace(0*ver_gradient, 1.5*ver_gradient, 10)
...
for i in range(1, row_num-1):
    for j in range(1, row_num-1):
        if surf_edges[i, j]>0:
            if (surf_edges[i-1, j]<1 and surf_edges[i+1, j]<1 and surf_edges[i, j-1]<1 and surf_edges[i, j+1]<1):
                checks_edges[i, j] = 0
        elif surf_edges[i, j]<1:
            if (surf_edges[i-1, j]>0 and surf_edges[i+1, j]>0) or (surf_edges[i, j-1]>0 and surf_edges[i, j+1]>0):
                checks_edges[i, j] = 1
